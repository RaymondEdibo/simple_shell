...
/**
 * _emy_puts - prints an input string
 * @str: the string to be printed
 *
 * Return: Nothing
 */
void _emy_puts(char *str)
{
	int i = 0;
...
/**
 * _emy_putchar - writes the character c to stderr
 * @c: The character to print
 *
 * Return: On success 1.
 * On error, -1 is returned, and errno is set appropriately.
 */
int _emy_putchar(char c)
{
	static int i;
 ...
 *        -1 on error
 */
void printerror(info_t *a, char *estr)
{
	_emy_puts(a->fname);
	_emy_puts(": ");
	printd(a->line_count, STDERR_FILENO);
	_emy_puts(": ");
	_emy_puts(a->argv[0]);
	_emy_puts(": ");
	_emy_puts(estr);
